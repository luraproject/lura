...
func TestEndpointHandler_ok(t *testing.T) {
	...
	testEndpointHandler(t, 10, p, expectedBody, "public, max-age=21600", "text/plain; charset=utf-8", http.StatusOK)
...
func TestEndpointHandler_incomplete(t *testing.T) {
	...
	testEndpointHandler(t, 10, p, expectedBody, "", "text/plain; charset=utf-8", http.StatusOK)
	time.Sleep(5 * time.Millisecond)
}
...
func TestEndpointHandler_noop(t *testing.T) {
	testEndpointHandler(t, 10, proxy.NoopProxy, "", "", "text/plain; charset=utf-8", http.StatusOK)
	time.Sleep(5 * time.Millisecond)
}
...
func setup(timeout time.Duration, p proxy.Proxy) (string, *http.Response, error) {
	...
	server := startGinServer(StreamEndpointHandler(endpoint, p))
